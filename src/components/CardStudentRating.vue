...
export default {
    ...
    methods: {
            ...
            for (let i = 0; i < this.update.length; i++) {
                ...
                api.put(`notes-store/${this.update[i].id}`, payload)
                    ...
                    .catch((error) => {
                        ...
                        this.$store.dispatch(
                            "messageError",
                            "Ocorreu um erro ao realizar a atualização das Notas"
                        );
                    });
            }
        },
        getNotes: function () {
            ...
            api.get(`notes-store/by-sprint/${this.sprintID}`)
                .then((notes) => {
                    let notasUsuario = notes.data;

                    for (let i = 0; i < notasUsuario.length; i++) {
                        if (
                            notasUsuario[i].idEvaluator == evaluatorId &&
                            notasUsuario[i].idEvaluated == this.estudanteID &&
                            notasUsuario[i].idGroup === this.idGroup
                        ) {
