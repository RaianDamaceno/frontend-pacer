  ...
  export default {
    ...
    methods: {
        async submitForm(){
          ...
          const CreateProject = {
            'openingDate':  this.initialDateNew.split("/").reverse().join("/"),
            'closeDate':    this.finalDateNew.split("/").reverse().join("/"),
            'description':    this.Description
          }
          console.log(this.initialDateNew);
          console.log(this.CreateProject);
           await axios.post('http://localhost:3000/project', CreateProject).then((response) => {
