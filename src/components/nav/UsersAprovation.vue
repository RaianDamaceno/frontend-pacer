<template>
  <v-row justify="center">
    ...
    <v-menu bottom offset-y> ProjectCriteriaAdd </v-menu>
    <v-dialog
      v-model="dialog"
      fullscreen
      hide-overlay
      ...
      <v-card tile>
        <v-toolbar flat dark color="primary">
          <v-btn icon dark @click="dialog = false">
            <v-icon id="close_icon_aprovation_users">mdi-close</v-icon>
          </v-btn>
          <v-toolbar-title>Usuários para Aprovação</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items >
            <v-btn v-if="this.teste.length > 0" dark text @click="submitForm"> Salvar </v-btn>
          </v-toolbar-items>
          <v-menu bottom right offset-y> </v-menu>
        </v-toolbar>
        <v-card-text>
          <v-list three-line subheader>
            <v-subheader>Lista de Novos Usuários para Aprovação</v-subheader>
            <v-data-table
              :headers="headers"
              :items="teste"
              :items-per-page="10"
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "RA", value: "document" },
        { text: "Email", value: "email" },
        { text: "Acesso", value: "role" },
